...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    // Check if there's a skill search from Recommendations
    const searchSkill = sessionStorage.getItem('searchSkill');
    if (searchSkill) {
      setSearchTerm(searchSkill);
      sessionStorage.removeItem('searchSkill'); // Clear it after use
      // Perform search after setting state
      setTimeout(() => {
  ...
  useEffect(() => {
    // Prevent the effect from running on the initial component mount
                    ...
                    >
                      ...
                      <Box mb={2}>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {userItem.averageRating?.toFixed(1) || "0.0"} (
                          {userItem.totalRatings || 0} reviews)
                        </Typography>
                      </Box>
                        ...
                        <Box display="flex" flexWrap="wrap" gap={0.5}>
                          ...
                          {userItem.skillsOffered?.length > 3 && (
                            <Chip
                              ...
                              color="primary"
                              onClick={(e) =>
                                handleShowMoreSkills(e, userItem.skillsOffered)
                              }
                              sx={{ cursor: "pointer" }}
                            />
                          )}
          ...
          <DialogContent dividers>
            {selectedUserDetails && (
              <Box>
                ...
                {/* Skills */}
                {selectedUserDetails.skillsOffered &&
                  selectedUserDetails.skillsOffered.length > 0 && (
                    <Box mb={3}>
                      <Typography
                        variant="subtitle2"
                        color="text.secondary"
                      ...
                      <Box display="flex" flexWrap="wrap" gap={1}>
                        {selectedUserDetails.skillsOffered.map(
                          (skill, index) => (
                            <Chip
                              key={index}
                              label={skill}
                ...
                {selectedUserDetails.skillsWanted &&
                  selectedUserDetails.skillsWanted.length > 0 && (
                    <Box mb={3}>
                      <Typography
                        variant="subtitle2"
                        color="text.secondary"
                      ...
                      <Box display="flex" flexWrap="wrap" gap={1}>
                        {selectedUserDetails.skillsWanted.map(
                          (skill, index) => (
                            <Chip
                              key={index}
                              label={skill}
                    ...
                    <Grid item xs={6}>
                      ...
                      >
                        <Typography variant="h4" color="warning.main">
                          {selectedUserDetails.averageRating
                            ? selectedUserDetails.averageRating.toFixed(1)
                            : "N/A"}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Average Rating
